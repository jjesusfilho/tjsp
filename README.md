...
Para baixar informações processuais o ESAJ exige autenticação em duas
etapas,i.e., além de usuário e senha, você tem de ter o pacote tjsp a
obter um token enviado para seu email.

No momento, é possível obter o token do gmail e do outlook (personal e
business). Para tanto, você deve seguir as instruções dos pacotes
[gmailr](https://github.com/r-lib/gmailr) ou do pacote
[Microsoft365R](https://github.com/Azure/Microsoft365R) para autorizar o
acesso automático ao seu email e extração do token.
...
Alternativamente, se você possui certificado digital A1 (arquivo `.pfx`
ou `.p12`), pode autenticar diretamente com ele, sem precisar do token
por email:

``` r
tjsp_autenticar(metodo = "certificado", certificado = "caminho/do/certificado.pfx")
```

A senha do certificado pode ser informada no argumento
