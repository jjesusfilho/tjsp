                             ...
                             diretorio = ".")
{
  ...
  purrr::map_dfr(processos, purrr::possibly(~{
    ...
    query1 <- list(cbPesquisa = "NUMPROC", conversationId = "",
    ...
    conteudo1 %>%
      purrr::map_dfr(purrr::possibly(~{
        ...
        doc_num <- doc %>% xml2::xml_attr("href") %>%
        ...
        uri2 <- purrr::map_chr(doc_num, ~{
          ...
          uri_parseada$query <- list(cdDocumento = .x,
        ...
        purrr::map2(uri3, data_decisao, ~{
          httr::GET(.x, httr::write_disk(paste0(diretorio,
                                                "/", stringr::str_replace_all(.y, "/", "_"),
                                                "_", stringr::str_remove_all(p, "\\D+"), ".pdf"),
                                         overwrite = TRUE))
        })

        tibble::tibble(processo = p, data_jugalmento = data_decisao,
