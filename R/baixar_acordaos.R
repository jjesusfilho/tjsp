...
baixar_acordaos <- function(processos = NULL,
  ...
  purrr::map_dfr(processos, purrr::possibly( ~ {
    ...
    if (xml2::xml_find_first(conteudo1, "boolean(//div[@id='listagemDeProcessos'])")) {
      conteudo1 <-
        xml2::xml_find_all(conteudo1, "//a[@class='linkProcesso']") %>%
        xml2::xml_attr("href") %>%
    ...
    conteudo1 %>%
      purrr::map_dfr(purrr::possibly( ~ {
        ...
        doc <- cc %>%
          xml2::xml_find_all(
            "//tr/td/a[contains(.,'Acordão Finalizado')]|//tr/td/a[contains(.,'Julgado virtualmente')]"
          )
        ...
        uri2 <- purrr::map_chr(doc_num,  ~ {
          uri_parseada <-
            httr::parse_url("https://esaj.tjsp.jus.br/cposg/verificarAcessoMovimentacao.do?")
        ...
        purrr::map2(uri3, data_decisao,  ~ {
          httr::GET(.x, httr::write_disk(
            paste0(
              ...
              ## Coloca subscrito no lugar de barra
              "_",
              stringr::str_remove_all(p, "\\D+"),
              ## Remove o que não é número dos números dos processo.
              ".pdf"
            ),
            overwrite = TRUE
          ))
