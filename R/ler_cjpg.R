...
df<- purrr::map_dfr(a,purrr::possibly(~{
 ...
 resposta <- .x %>%
   ...
   unlist()
      ...
      processo <- stringi::stri_extract_first_regex(resposta, "\\d+-\\d{2}\\.\\d{4}\\.\\d\\.\\d{2}\\.\\d{4}",
                                           omit_no_match = F)

      classe <- stringi::stri_extract_first_regex(resposta, "Classe:.*",
                                         omit_no_match = F)

      assunto <- stringi::stri_extract_first_regex(resposta, "Assunto:.*",
                                          omit_no_match = F)
      magistrado <- stringi::stri_extract_first_regex(resposta, "Magistrado:.*",
                                             omit_no_match = F)
      comarca <- stringi::stri_extract_first_regex(resposta, "Comarca:.*",
                                          omit_no_match = F)
      foro <- stringi::stri_extract_first_regex(resposta, "Foro:.*", omit_no_match = F)
      vara <- stringi::stri_extract_first_regex(resposta, "Vara:.*", omit_no_match = F)
      disponibilizacao <- stringi::stri_extract_first_regex(resposta, "Data\\s+de\\s+Disponibilização:.*",
                                                   omit_no_match = F)
  ...
  mutate(disponibilizacao=lubridade::dmy(disponibilizacao))
}
