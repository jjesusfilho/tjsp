...
tjpg<-function (url){
  ...
  val <- xpathApply(b, "//*[@bgcolor='#EEEEEE']", xmlValue,
                    trim = T)[[1]]
  ...
  for (i in 1:max_pag) {
    tryCatch({
      ...
      s <- xpathApply(d, "//*[@id='divDadosResultado']/table//td//td[@align='left']",
                      xmlValue, trim = T)
      s<-stri_replace_all_regex(s,"\\s+"," ")
      s<-paste0(s,collapse = "\n")
      s<-unlist(stri_split_regex(s,"\n(?=\\d{4,})"))

      processo<-stri_extract_first_regex(s,"\\d+-\\d{2}\\.\\d{4}\\.\\d\\.\\d{2}\\.\\d{4}",omit_no_match=F)
      classe<-stri_extract_first_regex(s,"Classe:.*",omit_no_match=F)
      assunto<-stri_extract_first_regex(s,"Assunto:.*",omit_no_match=F)
      magistrado<-stri_extract_first_regex(s,"Magistrado:.*",omit_no_match=F)
      comarca<-stri_extract_first_regex(s,"Comarca:.*",omit_no_match=F)
      foro<-stri_extract_first_regex(s,"Foro:.*",omit_no_match=F)
      vara<-stri_extract_first_regex(s,"Vara:.*",omit_no_match=F)
      disponibilizacao<-stri_extract_first_regex(s,"Data\\sde\\sDisponibilização:.*",omit_no_match=F)
      julgado<-stri_extract_last_regex(s,"(?<=\n).*",omit_no_match=F)

      df1 <- data.frame(processo,classe,assunto,magistrado,comarca,foro,vara,disponibilizacao,julgado,stringsAsFactors = F)
